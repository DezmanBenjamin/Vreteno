...
#define SEND_MILLIS 8   // send commands to hoverboard every SEND_MILLIS millisesonds
...
#ifdef ESP32
  const int pinRX = 4, pinTX = 17;   // Wemos S2 Mini
...
void loop()
{
  ...
  //int iSpeed = (map(analogRead(34) , 0, 4095, -500, 500)); 
  //int iSpeed = 300;
  int iSteer = 0;
  ...
  {
    HoverSend(oSerialHover,0,CLAMP(300,-1000,1000),wState);
  ...
  while (bReceived = Receive(oSerialHover,oHoverFeedback))
  {
    ...
    #endif
    {
      ...
      //Serial.println((float)oHoverFeedback.iSpeed/100.0);
      HoverLog(oHoverFeedback);

      modifier = (float)(oHoverFeedback.iSpeed-target_speed)/target_speed;
      //iSpeed -= modifier;
      if(oHoverFeedback.iSpeed > target_speed){
        iSpeed -= (int)50*modifier;
        Serial.print("\nModifier");
  ...
  if (iNow > iNext)
  {
    //DEBUGN("time",iNow)
      ...
      //if (bReceived)  // Reply only when you receive data
       ...
       //HoverSend(oSerialHover,0,300,wState,wState);
